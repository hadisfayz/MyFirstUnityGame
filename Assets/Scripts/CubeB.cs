...
public class CubeB : MonoBehaviour {
    ...
    void Awake() {
        ...
        input.GameTrak.RightXAxis.performed += ctx => RepositionCube();
        input.GameTrak.RightYAxis.performed += ctx => RepositionCube();
        input.GameTrak.RightZAxis.performed += ctx => RepositionCube();
    }
    ...
    Sphere GetJoystickInput() {
        ...
        sphereCoords.r = -input.GameTrak.RightZAxis.ReadValue<float>(); // -1 to 1
        // r was inverted to make the cube move in the same direction as the controller
        sphereCoords.theta = input.GameTrak.RightXAxis.ReadValue<float>(); // -1 to 1
        sphereCoords.phi = input.GameTrak.RightYAxis.ReadValue<float>(); // -1 to 1
